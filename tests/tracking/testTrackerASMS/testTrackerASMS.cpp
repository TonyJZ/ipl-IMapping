// testTrackerASMS.cpp : Defines the entry point for the console application.
//
#include <iostream>
#include <vector>
#include <string>
...
{
	printf("test tracker ASMS: Appropolis Inc.\n");
	printf("usage:\n");
	printf("video \n");
	printf("---------------------------------------------\n");
	if (wait)
...
int main(int argc, char * argv[])
{
	...
	ColorTracker * asmsTracker = NULL;
	bool g_trackerInitialized = false;

	asmsTracker = new ColorTracker();
	...
	asmsTracker->init(frame, roi.x, roi.y, roi.x+roi.width, roi.y+roi.height);
	...
	for (;; ) {
		...
		bb = asmsTracker->track(frame);

		roi.x = bb->x; roi.y = bb->y; roi.width = bb->width; roi.height = bb->height;
