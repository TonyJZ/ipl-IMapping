...
//    }
  ...
  std::ifstream data("D:/code_ipl/3rdparty/tracking/source/MOT/PHD_filter/SMC-LMB/dataset_videos/PETS09-S2L1/det/det.txt");
  ...
  cv::VideoCapture vc("D:/code_ipl/3rdparty/tracking/source/MOT/PHD_filter/SMC-LMB/dataset_videos/PETS09-S2L1/img1/%6d.jpg");
  ...
  while (!frame.empty()) {
    ...
    for (auto &d : detections[frame_cnt]) {
      ...
      //            rect.y = std::max(0.0, (*(Zk.end()-1))(1) -
      //            rect.height/2.0);
      //            cv::rectangle(frame, rect.tl(), rect.br(),
      //            cv::Scalar(0,0,255), 3);

	  cv::rectangle(frame, d.tl(), d.br(), cv::Scalar(255, 255, 0), 3);
    }
  ...
  //  std::cout << "WC_FPS: " << worst_case_fps << " for frame: " << std::endl;
