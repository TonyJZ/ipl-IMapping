...
murtyKBestAssignment(const arma::fmat &CM, unsigned m,
  ...
  //float *assignment = new float[CM.n_rows];
  std::vector<float> assignment;
  assignment.resize(CM.n_rows);
  float cost;
  assignmentoptimal(&assignment[0], &cost, (float *)CM.memptr(), CM.n_rows,
                    CM.n_cols);
  
//   int i = 0;
//   for (; i < CM.n_rows; i++) 
  for (auto &a : assignment) {
	  //float a = assignment[i];
  ...
  while (potential_assignments.size()) {
    ...
    for (size_t i = 0; i < current_optimal_assign.second.first.size(); i++) {
      ...
      if (j >= 0) {
		...
		//float *assignment_tmp = new float[CM_tmp.n_rows];
		std::vector<float> assignment;
		assignment.resize(CM_tmp.n_rows);
        float cost;
        assignmentoptimal(&assignment[0], &cost, CM_tmp.memptr(), CM_tmp.n_rows,
                          CM_tmp.n_cols);
        for (auto &a : assignment) {
          the_optimal_assignment.first.emplace_back(a);
        }
